...
temp17<-group_by(temp,position,hour,location)
weather_sum<-dplyr::summarize(temp1,mn.t=mean(Temperature),mn.h=mean(Humidity))
weather_sum$hour=as.integer(temp1_sum$hour)
?dplyr::summarise

posthoc.kruskal.dunn.test(temp$Temperature~temp$position)

ggplot(temp17,aes(x=hour,y=Temp,color=position))+geom_smooth()

day <-  filter(temp17, hour >= 6, hour <= 20)
ggplot(temp17,aes(x=hour,y=Temperature,color=position))+
  geom_smooth(method='gam',formula=y~s(x))
flevels <- c("treestand","edge","Twenty Meters","grassland")
temp17$position <-factor(temp17$position,levels=flevels)



mod<-gam(Temperature~s(hour,by=position,k=23)+
           position+s(location,bs="re"),data=temp17,
         family=scat(link="log"))
summary(mod)
par(mfrow=c(1,1))
plot(mod,shift=TRUE,all.terms = TRUE,pages=2)

pdat <- with(temp17, expand.grid(position = levels(position),
                                 location = levels(location),
                                 hour  = seq(min(hour), max(hour), length = 200)
)
)

pdat <- transform(pdat, pred = predict(mod, newdata = pdat, type = "response"))

ylims <- with(temp17, range(Temperature))

## draw base plot
plot(Temperature ~ hour, data = temp17, xaxt = 'n')
levs <- levels(temp17[['position']])
cols <- c('red', 'green', 'blue','yellow')

## add the fitted lines
for (l in seq_along(levs)) {
  dd <- subset(pdat, position == levs[l])
  lines(pred ~ hour, data = dd, col = cols[[l]])
}
...
ggsave("temp.transects.png",plot=last_plot(),scale=1,width=5,height=5,units="in",dpi=300)
...
b<-getViz(mod)
check.gamViz(b)
vis.gam(mod,view="hour")

?plot.gam

?plotGAM

gg <- plotGAM(mod,smooth.cov="hour", groupCovs="position",rawOrFitted = "fitted",
              plotCI = T,orderedAsFactor = TRUE)



night <-  filter(temp17, hour < 6 | hour > 20)
ggplot(night,aes(x=as.factor(hour),y=Temperature,color=position))+geom_boxplot()
mod<-aov(data=night, Temp ~ position)
summary(mod)
TukeyHSD(mod)
temp17$Temp
qqnorm(log(night$Temp))
qqline(log(night$Temp))
...
temp16<-group_by(temp,position,hour,location,depth)
